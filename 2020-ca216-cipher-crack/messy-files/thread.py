...
global list_of_training_types
list_of_training_types = []
global list_of_cipher_types
list_of_cipher_types = []
...
def cipher_Thread():
    ...
    with open(sys.argv[1], "r") as ciphertext:
        ...
        list_of_cipher_types.append(uni_ciphertext_word_freq)
        list_of_cipher_types.append(bi_ciphertext_word_freq)
        list_of_cipher_types.append(tri_ciphertext_word_freq)

        print(list_of_cipher_types)
...
def trianing_Thread():
    ...
    with open(sys.argv[2], "r") as trainingtext:
        ...
        list_of_training_types.append(uni_trainingtext_word_freq)
        list_of_training_types.append(bi_trainingtext_word_freq)
        list_of_training_types.append(tri_trainingtext_word_freq)

        print(list_of_training_types)

def main():
    
    with open("output.txt", "w") as output, open("key.txt", "w") as key_file:

        threads = []
        
        cipher_thread_executor = threading.Thread(target=cipher_Thread)
        training_thread_executor = threading.Thread(target=trianing_Thread)

        threads.append(cipher_thread_executor)
        threads.append(training_thread_executor)
